...
import (
	...
	jsonParser "poc_jsonParser/utils/jsonParser_v2"
	"poc_jsonParser/utils/jsonParser_v3"
...
func TestGetField(t *testing.T) {
	...
	res := jsonParser.Get(configuration.JsonStr, "bookingDateTime")
...
func BenchmarkTestGetField(b *testing.B) {
	// b.N = 1_000_000
	for i := 0; i < b.N; i++ {
		jsonParser.Get(configuration.JsonStr, "action")
	}
}

func BenchmarkTestGetField_v3(b *testing.B) {
	// b.N = 1_000_000
	for i := 0; i < b.N; i++ {
		jsonParser_v3.Get([]byte(configuration.JsonStr), "action")
	}
}
...
func BenchmarkTestGetField_Raw(b *testing.B) {
	// b.N = 1_000_000
	for i := 0; i < b.N; i++ {
		jsonParser.Get(configuration.JsonStr, "action", jsonParser.ParserOption{IsRaw: true})
	}
}
...
func TestGetNestedField(t *testing.T) {
	res := jsonParser.Get(configuration.JsonStr, "customer.Name1", jsonParser.ParserOption{IsRaw: false})
	if res.Raw != `"1dxmcz"` {
...
func TestGetArrayIndex(t *testing.T) {
	res := jsonParser.Get(configuration.JsonStr, "serviceOrderJobs[1]", jsonParser.ParserOption{IsRaw: false})
