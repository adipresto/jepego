...
func TestGetField_v3(t *testing.T) {
	...
	if string(res.Raw) != "2025-01-23 10:00:00" {
		t.Errorf("expected \"2025-01-23 10:00:00\", got %s", string(res.Raw))
	}
}
func TestGetNestedField_v3(t *testing.T) {
	res := jsonParser.Get([]byte(configuration.JsonStr), "customer.Name1")
	if string(res.Raw) != "1dxmcz" {
		t.Errorf("expected \"1dxmcz\", got %s", string(res.Raw))
	}
}
